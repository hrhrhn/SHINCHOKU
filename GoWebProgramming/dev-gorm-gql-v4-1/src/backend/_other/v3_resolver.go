...
import (
	"context"
	"dev-gorm-gql-v3/backend/database"
	"dev-gorm-gql-v3/backend/models"
	"errors"
	"log"
	"strconv"
...
func (r *mutationResolver) FixTodo(ctx context.Context, input int) (*models.Todo, error) {
	...
	todo, err := database.NewTodoDao(&r.DB).FixOne(input)
	if err != nil {
		return nil, err
	}
	if todo == nil {
...
func (r *mutationResolver) DeleteUser(ctx context.Context, input *int) (bool, error) {
	// panic("not implemented")
	todos, err := database.NewTodoDao(&r.DB).FindByUserID(*input)
	if err != nil {
		return false, err
	}

	for _, todo := range todos {
		err := database.NewTodoDao(&r.DB).DeleteOne(todo.ID)
		if err != nil {
			return false, err
		}
	}
