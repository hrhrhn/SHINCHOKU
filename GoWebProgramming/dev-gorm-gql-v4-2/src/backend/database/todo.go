...
type TodoDao interface {
	...
	FindByDetailID(detailID int) (*Todo, error)
	FindOne(id int) (*Todo, error)
	DeleteOne(id int) error
	DoneOne(id int) (*Todo, error)
	FixOne(id int, content string) (*Todo, error)
}
...
func (d *todoDao) FindByDetailID(detailID int) (*Todo, error) {
	var todos []*Todo
	res := d.db.Table("todos_v4").
		Select("todos_v4.*").
		Joins("LEFT JOIN details_v4 ON details_v4.todo_id = todos_v4.id").
...
func (d *todoDao) FixOne(id int, content string) (*Todo, error) {
	todo := Todo{}
	// fmt.Println("call for done.")
	res := d.db.Where("id = ?", id).Find(&todo).Update("content", content)
	// fmt.Println("res", res)
	if err := res.Error; err != nil {
		return nil, err
	}
