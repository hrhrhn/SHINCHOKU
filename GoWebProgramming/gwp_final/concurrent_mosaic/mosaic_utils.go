...
// 画像をリサイズ
func resize(in image.Image, netWidth int) image.NRGBA {
	bounds := in.Bounds()
	ratio := bounds.Dx() / netWidth
...
// タイル画像のデータベースをメモリ内に作成
func tilesDB() map[string][3]float64 {
	fmt.Println("Start populating tiles db ...")
	db := make(map[string][3]float64)
	files, _ := ioutil.ReadDir("../simple_mosaic/tiles")
	for _, f := range files {
		name := "../simple_mosaic/tiles/" + f.Name()
...
// (r, g, b)空間におけるユークリッド距離
func distance(p1 [3]float64, p2 [3]float64) float64 {
	return math.Sqrt(sq(p2[0]-p1[0]) + sq(p2[1]-p1[1]) + sq(p2[2]-p1[2]))
}

// タイル画像のデータベースからtargetと最も近いファイルを探して返す
